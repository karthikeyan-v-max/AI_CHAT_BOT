...
router.post("/text", async(res,req)=>{
    try{
        ...
        res.status(200).json({ text: response.data.choice[0].text })
    }catch(error){
...
router.post("/code", async(res,req)=>{
try{
    const { code, activeChatId } = req.body
    const response = await openai.createCompletion({
        modal : "test-davinci-002",
        prompt: text,
        temperature: 0.5,
        max_tokens: 2048,
        top_p: 1,
...
router.post("/assist", async(res,req)=>{
    try{
        const { text } = req.body
        const response = await openai.createCompletion({
            modal : "test-davinci-002",
